...
def merge_lora_model(model_path: str, output_path: str = None) -> str:
    """
    Merge LoRA adapter with base model and return the path to merged model.
    ...
    """
    if not is_lora_model(model_path):
        return model_path
    ...
    if output_path is None:
        output_path = model_path + "_merged"
    ...
    print(f"LoRA model merged successfully. Merged model saved at: {output_path}")
    return output_path
...
def eval(
    ...
    # Check if model is LoRA and merge if necessary
    merged_model_path = merge_lora_model(base_model)
